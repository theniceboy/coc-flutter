...
export const registerHotReloadProvider = (): Disposable => {
	const enableHotReload = workspace
		.getConfiguration('flutter')
		.get<boolean>('provider.hot-reload', true);
	...
	if (enableHotReload) {
		subscription = workspace.onDidSaveTextDocument((doc: TextDocument) => {
			if (doc.languageId === 'dart' && devServer.state) {
				...
				const hasErrors =
					diagnostics &&
					diagnostics.find((d) => d.source === 'dart' && d.severity === DiagnosticSeverity.Error) !=
						null;
				if (!hasErrors) {
	...
	const configChangeSubs = workspace.onDidChangeConfiguration((e: ConfigurationChangeEvent) => {
		if (e.affectsConfiguration('flutter')) {
			const isEnableHotReload = workspace
				.getConfiguration('flutter')
				.get<boolean>('provider.hot-reload', true);
			// disable hot reload
